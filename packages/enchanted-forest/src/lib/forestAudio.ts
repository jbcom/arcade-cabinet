...
class ForestAudioEngine {
  ...
  async initialize(): Promise<void> {
    ...
    this.mainReverb = new Tone.Reverb({ decay: 4, wet: 0.5, preDelay: 0.1 }).connect(this.masterVolume);
    this.mainDelay = new Tone.FeedbackDelay({ delayTime: "8n.", feedback: 0.3, wet: 0.2 }).connect(this.mainReverb);
    this.chorus = new Tone.Chorus({ frequency: 0.5, delayTime: 3.5, depth: 0.7, wet: 0.3 }).connect(this.mainDelay);
    this.ambientFilter = new Tone.Filter({ frequency: 800, type: "lowpass", Q: 2 }).connect(this.mainReverb);
    this.ambientDrone = new Tone.Synth({ oscillator: { type: "sine" }, envelope: { attack: 2, decay: 1, sustain: 0.8, release: 4 }, volume: -20 }).connect(this.ambientFilter);
    this.windFilter = new Tone.AutoFilter({ frequency: 0.1, baseFrequency: 200, octaves: 4, depth: 0.8 }).connect(this.mainReverb);
    this.windFilter.start();
    this.windNoise = new Tone.Noise({ type: "pink", volume: -30 }).connect(this.windFilter);
    this.melodySynth = new Tone.PolySynth(Tone.Synth, { oscillator: { type: "triangle" }, envelope: { attack: 0.02, decay: 0.3, sustain: 0.2, release: 1 }, volume: -8 }).connect(this.chorus);
    this.padSynth = new Tone.PolySynth(Tone.Synth, { oscillator: { type: "sine" }, envelope: { attack: 1, decay: 2, sustain: 0.6, release: 3 }, volume: -15 }).connect(this.mainReverb);
    this.bellSynth = new Tone.PolySynth(Tone.Synth, { oscillator: { type: "sine" }, envelope: { attack: 0.001, decay: 1.5, sustain: 0, release: 2 }, volume: -10 }).connect(this.mainDelay);
  ...
  startAmbient(): void {
    ...
    this.ambientSequence = new Tone.Sequence((time, note) => {
  ...
  playTouchNote(x: number, y: number): void {
  ...
  playSpellEffect(type: ToneType): void {
    ...
    switch (type) {
      case "shield":
        ...
        SHIELD_SCALE.forEach((note, i) => this.bellSynth?.triggerAttackRelease(note, "4n", now + i * 0.1, 0.5));
        break;
      case "heal":
        HEALING_SCALE.forEach((note, i) => this.melodySynth?.triggerAttackRelease(note, "8n", now + i * 0.15, 0.6));
        this.padSynth?.triggerAttackRelease(["E3", "G#3", "B3"], "1n", now + 0.3, 0.4);
        break;
      case "purify":
        this.bellSynth?.triggerAttackRelease(["D5", "F#5", "A5"], "4n", now, 0.8);
        PURIFY_SCALE.forEach((note, i) => this.melodySynth?.triggerAttackRelease(note, "16n", now + i * 0.08, 0.5));
        break;
      case "damage":
        this.padSynth?.triggerAttackRelease(["C3", "Db3", "Gb3"], "8n", now, 0.4);
        break;
      case "victory":
        ["C4", "E4", "G4", "C5"].forEach((note, i) => this.bellSynth?.triggerAttackRelease(note, "2n", now + i * 0.2, 0.7));
        this.padSynth?.triggerAttackRelease(["C3", "G3", "C4", "E4", "G4"], "1n", now + 0.5, 0.5);
        break;
      case "defeat":
        ["E4", "D4", "C4", "B3", "A3"].forEach((note, i) => this.melodySynth?.triggerAttackRelease(note, "4n", now + i * 0.4, 0.4));
  ...
  playGestureSequence(points: { x: number; y: number }[], type: ToneType): void {
    ...
    const scale = type === "heal" ? HEALING_SCALE : type === "shield" ? SHIELD_SCALE : type === "purify" ? PURIFY_SCALE : FOREST_SCALE;
