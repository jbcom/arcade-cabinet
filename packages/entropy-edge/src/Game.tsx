...
export default function Game() {
  ...
  useEffect(() => {
    ...
    dirLight.position.set(10, 25, 10); dirLight.castShadow = true; dirLight.shadow.mapSize.set(1024, 1024);
    scene.add(dirLight);

    const gridShaderMat = new THREE.ShaderMaterial({
      uniforms: { time: { value: 0 }, glowColor: { value: new THREE.Color(0x00b3cc) }, pulseSpeed: { value: 2.0 } },
    ...
    gridFloor.rotation.x = -Math.PI / 2; gridFloor.position.y = -0.51; scene.add(gridFloor);

    const blockGeo = new THREE.BoxGeometry(BLOCK_SIZE, BLOCK_SIZE, BLOCK_SIZE);
    const blockMat = new THREE.MeshPhysicalMaterial({ color: 0x111822, metalness: 0.7, roughness: 0.2, clearcoat: 1.0, flatShading: true });
    ...
    const holoMat = new THREE.MeshBasicMaterial({ color: 0x00e5ff, transparent: true, opacity: 0.5 });
    const nodeGeo = new THREE.IcosahedronGeometry(0.7, 1);
    const nodeMat = new THREE.MeshStandardMaterial({ color: 0xff0055, emissive: 0xff0055, emissiveIntensity: 1.5, wireframe: true });
    ...
    let targetNode = null, targetPillar = null;
    let targetLight = new THREE.PointLight(0xff0055, 4.0, 20);
    ...
    uiLayer.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;display:flex;flex-direction:column;justify-content:space-between;padding:20px;box-sizing:border-box;z-index:20;font-family:Rajdhani,sans-serif;color:white;';
    ...
    scorePanel.innerHTML = '<div style="font-size:14px;color:#a0b4c8;letter-spacing:2px;margin-bottom:2px;font-weight:700;">TOTAL SCORE</div><div id="score-display" style="font-size:32px;font-weight:700;color:#00e5ff;">0 PTS</div>';
    hudTop.appendChild(scorePanel);

    const goalPanel = mkPanel('goal-panel','position:absolute;top:20px;left:50%;transform:translateX(-50%) translateY(-50px);background:'+panelBg+';border:2px solid #ffcc00;padding:10px 30px;border-radius:4px;box-shadow:0 10px 30px rgba(255,204,0,0.2);backdrop-filter:blur(10px);text-align:center;opacity:0;transition:transform 0.6s cubic-bezier(0.175,0.885,0.32,1.275),opacity 0.6s ease;');
    goalPanel.innerHTML = '<div id="sector-title" style="font-size:14px;color:#ffcc00;letter-spacing:3px;font-weight:700;text-transform:uppercase;">Sector 1</div><div id="anchor-tracker" style="font-size:32px;color:white;font-weight:700;margin-top:4px;">ANCHORS: 0 / 3</div>';
    ...
    timerContainer.innerHTML = '<div style="font-size:16px;color:white;text-transform:uppercase;letter-spacing:3px;">STABILITY</div><div id="timer-display" style="font-size:64px;font-weight:700;color:white;line-height:1;">20.0s</div><div id="resonance-bar-container" style="position:absolute;bottom:0;left:0;width:100%;height:6px;background:rgba(255,255,255,0.1);"><div id="resonance-fill" style="height:100%;width:0%;background:#00e5ff;transition:width 0.1s linear,background 0.3s ease;box-shadow:0 0 10px #00e5ff;"></div></div>';
    hudTop.appendChild(timerContainer);

    const distPanel = mkPanel('distance-panel','text-align:right;border-color:rgba(255,0,85,0.4);');
    distPanel.innerHTML = '<div style="font-size:14px;color:#a0b4c8;letter-spacing:2px;margin-bottom:2px;font-weight:700;">ANCHOR DISTANCE</div><div id="distance-display" style="font-size:32px;color:white;font-weight:700;">0.0m</div>';
    hudTop.appendChild(distPanel);

    const dangerVignette = document.createElement('div');
    dangerVignette.id = 'danger-vignette';
    dangerVignette.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;pointer-events:none;transition:box-shadow 0.2s ease;z-index:10;';
    container.appendChild(dangerVignette);

    const successFlash = document.createElement('div');
    successFlash.id = 'success-flash';
    successFlash.style.cssText = 'position:absolute;top:0;left:0;width:100%;height:100%;background:white;opacity:0;pointer-events:none;transition:opacity 0.5s ease;z-index:15;';
    container.appendChild(successFlash);

    const crosshair = document.createElement('div');
    crosshair.id = 'crosshair';
    crosshair.style.cssText = 'position:absolute;top:50%;left:50%;width:10px;height:10px;border:2px solid #00e5ff;border-radius:50%;transform:translate(-50%,-50%);pointer-events:none;z-index:15;opacity:0;transition:opacity 0.5s ease;';
    container.appendChild(crosshair);

    const comboPopup = document.createElement('div');
    comboPopup.id = 'combo-popup';
    comboPopup.style.cssText = 'position:absolute;top:50%;left:50%;transform:translate(-50%,-150%);color:#ffcc00;font-size:40px;font-weight:700;text-transform:uppercase;pointer-events:none;opacity:0;transition:all 0.3s ease;z-index:25;font-family:Rajdhani,sans-serif;';
    comboPopup.textContent = 'Resonance Max';
    container.appendChild(comboPopup);

    const tutorialOverlay = document.createElement('div');
    tutorialOverlay.id = 'tutorial-overlay';
    tutorialOverlay.style.cssText = 'position:absolute;top:75%;left:50%;transform:translate(-50%,-50%);text-align:center;pointer-events:none;z-index:20;opacity:0;transition:opacity 0.5s ease;font-family:Rajdhani,sans-serif;';
    ...
    const btnStyle = 'background:#00e5ff;color:rgba(10,15,25,0.9);font-family:Rajdhani,sans-serif;font-size:30px;font-weight:700;text-transform:uppercase;padding:20px 60px;border:none;border-radius:4px;cursor:pointer;box-shadow:0 10px 40px rgba(0,229,255,0.4);letter-spacing:4px;margin-top:20px;';

    const startMenu = mkMenu('start-menu', `
    ...
    const levelCompleteMenu = mkMenu('level-complete-menu', `
    ...
    const gameOverMenu = mkMenu('game-over-menu', `
    ...
    function updateGoalUI() {
      ...
      if (trackerEl) trackerEl.innerText = `ANCHORS: ${state.anchorsSecuredThisLevel} / ${state.anchorsRequired}`;
    }
    ...
    function generateNextNode(baseX, baseZ) {
      if (targetNode) { scene.remove(targetNode); scene.remove(targetPillar); }
      ...
      const pillarMat = new THREE.MeshBasicMaterial({ color: 0xff0055, transparent: true, opacity: 0.4 });
    ...
    function initiateBootSequence() {
      ...
      if (sm) { sm.style.opacity = '0'; sm.style.pointerEvents = 'none'; }
      setTimeout(() => { const p = getElementById('score-panel'); if (p) { p.style.transform = 'translateY(0)'; p.style.opacity = '1'; } }, 800);
      setTimeout(() => { const p = getElementById('goal-panel'); if (p) { p.style.transform = 'translateX(-50%) translateY(0)'; p.style.opacity = '1'; } }, 1000);
      setTimeout(() => { const p = getElementById('distance-panel'); if (p) { p.style.transform = 'translateY(0)'; p.style.opacity = '1'; } }, 1200);
      setTimeout(() => {
        const t = getElementById('timer-container'); if (t) { t.style.transform = 'scale(1)'; t.style.opacity = '1'; }
        const c = getElementById('crosshair'); if (c) c.style.opacity = '1';
        ...
        if (tutOverlay) {
          ...
          if (tutInst) tutInst.innerText = `Goal: Secure ${state.anchorsRequired} Anchors. Click core block to start.`;
        }
    ...
    function triggerLevelComplete() {
      ...
      if (menu) { menu.style.opacity = '1'; menu.style.pointerEvents = 'all'; }
      const stats = getElementById('level-stats');
      if (stats) stats.innerText = `Anchors Secured: ${state.anchorsRequired}\nSector Score: ${state.score} PTS`;
    }

    function triggerNodeReached() {
      state.totalAnchors++; state.anchorsSecuredThisLevel++;
      ...
      updateGoalUI();
      const wave = new THREE.Mesh(new THREE.TorusGeometry(1, 0.2, 16, 64), new THREE.MeshBasicMaterial({ color: 0xff0055, transparent: true }));
      wave.position.copy(targetNode.position); wave.rotation.x = Math.PI / 2;
      scene.add(wave); state.shockwaves.push({ mesh: wave, life: 1.0 });
      for (let [key, mesh] of state.grid.entries()) {
        if (mesh.position.distanceTo(targetNode.position) > SAFE_ZONE_RADIUS) {
          state.grid.delete(key); state.fallingBlocks.push({ mesh, velocity: Math.random() * 2 });
        }
      }
      if (state.anchorsSecuredThisLevel >= state.anchorsRequired) { triggerLevelComplete(); }
      else { generateNextNode(targetNode.position.x, targetNode.position.z); }
    }
    ...
    function onPointerDown() {
      ...
      if (state.phase === 'CALIBRATING') {
        ...
        setTimeout(() => { const tutOverlay = getElementById('tutorial-overlay'); if (tutOverlay) tutOverlay.style.opacity = '0'; }, 3000);
      }
      const pos = hologram.position.clone();
      createBlock(pos.x, pos.y, pos.z);
      const now = performance.now() / 1000;
      if (now - state.lastPlacementTime < COMBO_WINDOW) { state.resonance = Math.min(1.0, state.resonance + 0.15); }
      else { state.resonance = 0.15; }
      state.lastPlacementTime = now;
      const multiplier = state.isResonanceMax ? 2.5 : 1.0;
      state.score += 10 * multiplier; state.time += 0.2 * multiplier; state.cameraShake = 0.05;
    ...
    function animate() {
      ...
      if (state.phase === 'MENU') {
        ...
        if (starterBlock) { const pulse = 1 + (Math.sin(elapsed * 4) + 1) * 0.1; starterBlock.scale.set(pulse, pulse, pulse); starterBlock.children[0].material.color.setHex(0xffffff); }
      }

      if (state.phase === 'SWOOPING') {
        state.swoopProgress += dt * 0.8;
        const easeOutCub = 1 - Math.pow(1 - state.swoopProgress, 3);
        const currentOrbitPos = new THREE.Vector3(Math.sin(elapsed * 0.2) * 12, 2 + Math.sin(elapsed * 0.5), Math.cos(elapsed * 0.2) * 12);
        ...
        const starterBlock = state.grid.get("0,0,0");
        if (starterBlock) { starterBlock.scale.set(1,1,1); starterBlock.children[0].material.color.setHex(0x00e5ff); }
      }
      ...
      if (state.phase === 'PLAYING' || state.phase === 'CALIBRATING') {
        ...
        if (state.phase === 'PLAYING') {
          ...
          if (state.time <= 5.0) {
            gridShaderMat.uniforms.glowColor.value.set(0xff1111); gridShaderMat.uniforms.pulseSpeed.value = 12.0;
            if (dv) dv.style.boxShadow = 'inset 0 0 150px rgba(255,17,17,0.6)';
            if (timerEl) timerEl.style.color = '#ff1111';
          } else if (state.time <= 15.0) {
            gridShaderMat.uniforms.glowColor.value.set(0xffcc00); gridShaderMat.uniforms.pulseSpeed.value = 6.0;
          ...
          const now = performance.now() / 1000;
          if (now - state.lastPlacementTime > COMBO_WINDOW) state.resonance = Math.max(0, state.resonance - dt * 0.5);
        }
        ...
        if (targetNode && dd) dd.innerText = `${cameraTarget.distanceTo(targetNode.position).toFixed(1)}m`;
        ...
        if (intersects.length > 0) {
          ...
          if (!state.grid.has(`${pos.x},${pos.y},${pos.z}`)) { hologram.position.copy(pos); hologram.visible = true; }
          else hologram.visible = false;
        } else hologram.visible = false;
      }

      gridFloor.position.x = cameraTarget.x; gridFloor.position.z = cameraTarget.z;
      state.grid.forEach(b => { if (b.scale.x < 1.0) b.scale.lerp(new THREE.Vector3(1,1,1), 0.15); });
      if (targetNode) { targetNode.rotation.y += dt * 2; targetNode.scale.setScalar(1 + Math.sin(elapsed * 5) * 0.1); }
      if (state.cameraShake > 0 && state.phase !== 'SWOOPING') { state.cameraShake -= dt; camera.position.x += (Math.random() - 0.5) * state.cameraShake; }
      state.shockwaves.forEach((sw, i) => { sw.life -= dt * 1.5; sw.mesh.scale.addScalar(dt * 30); sw.mesh.material.opacity = sw.life; if (sw.life <= 0) { scene.remove(sw.mesh); state.shockwaves.splice(i, 1); } });
      state.fallingBlocks.forEach((fb, i) => { fb.velocity -= dt * 15; fb.mesh.position.y += fb.velocity * dt; if (fb.mesh.position.y < -20) { scene.remove(fb.mesh); state.fallingBlocks.splice(i, 1); } });
    ...
    container.addEventListener('click', (e) => {
      ...
      if (tgt.id === 'next-level-btn') {
        state.level++; state.anchorsRequired += 1; state.anchorsSecuredThisLevel = 0;
        state.time = Math.max(10.0, 20.0 - (state.level * 1.5)); state.resonance = 0;
        const menu = getElementById('level-complete-menu');
        if (menu) { menu.style.opacity = '0'; menu.style.pointerEvents = 'none'; }
        resetForNextLevel();
      }
      if (tgt.id === 'restart-btn') {
        state.level = 1; state.anchorsRequired = 3; state.anchorsSecuredThisLevel = 0;
        state.time = 20.0; state.score = 0; state.totalAnchors = 0; state.resonance = 0;
        const goMenu = getElementById('game-over-menu');
        if (goMenu) { goMenu.style.opacity = '0'; goMenu.style.pointerEvents = 'none'; }
        resetForNextLevel();
      }
    });

    function resetForNextLevel() {
      state.grid.forEach(m => scene.remove(m)); state.grid.clear();
      state.fallingBlocks.forEach(fb => scene.remove(fb.mesh)); state.fallingBlocks = [];
  ...
  return <div ref={mountRef} style={{ width: '100%', height: '100vh', position: 'relative', overflow: 'hidden', background: '#e6eaf0' }} />;
}
