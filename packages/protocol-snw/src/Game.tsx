...
export default function Game() {
  ...
  useEffect(() => {
    ...
    styleEl.textContent = `
      ...
      #snw-root { margin: 0; overflow: hidden; background: #020205; font-family: 'Orbitron', sans-serif; user-select: none; touch-action: none; width: 100%; height: 100%; position: relative; }

      #snw-root #ui-layer { position: absolute; top: 0; left: 0; width: 100%; height: 100%; pointer-events: none; display: flex; flex-direction: column; justify-content: space-between; z-index: 10; }

      #snw-root #hud { padding: 20px; display: flex; justify-content: space-between; align-items: flex-start; opacity: 0; transition: opacity 1s; gap: 15px;}

      #snw-root .hud-panel { background: rgba(0, 10, 20, 0.85); padding: 10px 15px; border: 2px solid rgba(0,255,100,0.5); transform: skewX(-10deg); flex: 1; max-width: 250px; box-shadow: 0 0 15px rgba(0,0,0,0.8);}
      #snw-root .hud-label { font-size: 0.75rem; color: #fff; letter-spacing: 2px; margin-bottom: 5px; text-transform: uppercase; font-weight: bold;}
      #snw-root .hud-value { font-size: 1.4rem; color: #00ff44; font-weight: 900; text-shadow: 0 0 10px currentColor; }

      #snw-root .bar-wrap { width: 100%; height: 12px; background: #222; margin-top: 5px; position: relative; border: 1px solid #000; }
      #snw-root #hp-bar { width: 100%; height: 100%; background: #ff0044; box-shadow: inset 0 0 10px rgba(255,0,0,0.5); transition: width 0.1s linear; }
      #snw-root #xp-bar { width: 0%; height: 100%; background: #00ff44; box-shadow: inset 0 0 10px rgba(0,255,68,0.5); transition: width 0.2s ease-out; }

      #snw-root #timer-display { font-size: 2.5rem; color: #fff; text-shadow: 0 0 15px #fff; text-align: center; font-variant-numeric: tabular-nums; font-weight: 900;}
      #snw-root #wave-display { font-size: 1.1rem; color: #00ffcc; text-shadow: 0 0 10px #00ffcc; text-align: center; letter-spacing: 3px; margin-bottom: -5px; font-weight: bold;}

      #snw-root #boss-hud { position: absolute; top: 15%; width: 100%; display: flex; justify-content: center; opacity: 0; transition: opacity 0.5s; pointer-events: none;}
      #snw-root #boss-bar-wrap { width: 50%; height: 20px; background: #111; border: 2px solid #ff0044; transform: skewX(-15deg); box-shadow: 0 0 30px rgba(255,0,68,0.3); }
      #snw-root #boss-hp-bar { width: 100%; height: 100%; background: #ff0044; box-shadow: 0 0 20px #ff0044; transition: width 0.2s; }
      #snw-root #boss-label { color: #ff0044; font-size: 1.5rem; text-shadow: 0 0 15px #ff0044; text-align: center; margin-bottom: 5px; font-weight: 900; letter-spacing: 4px; }

      #snw-root #combat-text-layer { position: absolute; top:0; left:0; width: 100%; height: 100%; pointer-events: none; z-index: 12; overflow: hidden; }
      #snw-root .dmg-text {
          position: absolute; font-weight: 900; font-size: 1.2rem; text-shadow: -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 1px 1px 0 #000, 0px 0px 8px #000;
          pointer-events: none; transform: translate(-50%, -50%); z-index: 100; font-family: 'Orbitron', sans-serif;
          transition: top 0.6s ease-out, opacity 0.6s ease-out;
      }

      #snw-root #controls-area { pointer-events: none; display: flex; justify-content: flex-end; align-items: flex-end; padding: 40px; height: 120px; gap: 20px; }
      #snw-root .action-btn { border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 900; backdrop-filter: blur(5px); transition: 0.1s; pointer-events: auto; opacity: 0; user-select: none; cursor: pointer; }
      #snw-root #dash-btn { width: 80px; height: 80px; background: rgba(255, 255, 255, 0.1); border: 3px solid #fff; color: #fff; font-size: 1.1rem; box-shadow: 0 0 15px rgba(255, 255, 255, 0.3); margin-bottom: 10px; }
      #snw-root #dash-btn:active, #snw-root #dash-btn.active { background: #fff; color: #000; transform: scale(0.95); box-shadow: 0 0 30px #fff; }
      #snw-root #dash-btn.cooldown { opacity: 0.2 !important; pointer-events: none; }

      #snw-root #joystick-zone { position: absolute; top: 0; left: 0; width: 100%; height: 100%; z-index: 5; pointer-events: auto; }
      #snw-root .stick-base { position: absolute; width: 120px; height: 120px; border-radius: 50%; border: 2px solid rgba(255,255,255,0.3); background: rgba(255,255,255,0.05); transform: translate(-50%, -50%); pointer-events: none; display: none; }
      #snw-root .stick-knob { position: absolute; width: 50px; height: 50px; border-radius: 50%; background: #fff; transform: translate(-50%, -50%); pointer-events: none; display: none; box-shadow: 0 0 20px #fff; }

      #snw-root .screen { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(2, 8, 15, 0.95); display: flex; flex-direction: column; align-items: center; justify-content: center; pointer-events: auto; z-index: 20; backdrop-filter: blur(10px); }
      #snw-root .screen.hidden { display: none !important; }

      #snw-root h1 { font-size: 4rem; color: #fff; text-shadow: 0 0 40px #00ffcc; margin: 0; letter-spacing: 5px; text-transform: uppercase; text-align: center; }
      #snw-root h1 span { color: #ff0044; text-shadow: 0 0 40px #ff0044; }
      #snw-root h3 { color: #888; margin-bottom: 40px; letter-spacing: 4px; text-transform: uppercase; font-size: 1rem; }

      #snw-root .class-container { display: flex; gap: 20px; flex-wrap: wrap; justify-content: center; max-width: 1000px; }
      #snw-root .class-card { background: rgba(0,0,0,0.8); border: 2px solid #333; padding: 25px; width: 220px; cursor: pointer; transition: 0.2s; text-align: center; position: relative; overflow: hidden; }
      #snw-root .class-card:hover:not(.locked) { border-color: #00ffcc; background: rgba(0,255,204,0.1); transform: translateY(-10px); box-shadow: 0 10px 30px rgba(0,255,204,0.2); }
      #snw-root .class-card.locked { opacity: 0.5; cursor: not-allowed; filter: grayscale(100%); }
      #snw-root .locked-overlay { position: absolute; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0,0,0,0.8); display: flex; flex-direction: column; justify-content: center; align-items: center; color: #ff0044; font-weight: bold; font-size: 1rem; z-index: 2; padding: 10px; box-sizing: border-box;}

      #snw-root .card-title { color: #fff; font-size: 1.4rem; font-weight: 900; margin-bottom: 15px; letter-spacing: 1px;}
      #snw-root .card-stat { color: #aaa; font-size: 0.85rem; margin: 6px 0; font-weight: bold;}
      #snw-root .stat-val { color: #00ffcc; }

      #snw-root .upgrade-card { background: rgba(0,0,0,0.9); border: 2px solid #00ffcc; padding: 30px; margin: 10px; width: 240px; cursor: pointer; transition: 0.2s; text-align: center; box-shadow: 0 0 20px rgba(0,255,204,0.1); }
      #snw-root .upgrade-card:hover { background: rgba(0,255,204,0.2); transform: scale(1.05); box-shadow: 0 0 40px rgba(0,255,204,0.5); }
      #snw-root .upg-title { font-size: 1.3rem; color: #fff; font-weight: 900; margin-bottom: 10px; letter-spacing: 1px;}
      #snw-root .upg-desc { font-size: 1rem; color: #00ffcc; font-weight: bold;}

      #snw-root .btn { background: #ff0044; border: none; padding: 20px 60px; color: white; font-family: 'Orbitron'; font-size: 1.5rem; cursor: pointer; margin-top: 50px; font-weight: 900; letter-spacing: 3px; transition: 0.2s; border-radius: 5px; }
      #snw-root .btn:hover { background: #fff; color: #ff0044; box-shadow: 0 0 40px #ff0044; }

      #snw-root #msg-overlay { position: absolute; top: 30%; width: 100%; text-align: center; pointer-events: none; z-index: 15; font-size: 3rem; font-weight: 900; color: #ff0044; text-shadow: 0 0 40px red, 0 0 10px #fff; opacity: 0; transition: opacity 0.5s, transform 0.5s; letter-spacing: 10px; transform: scale(1.5); }
      #snw-root #achievement-overlay { position: absolute; top: 10%; width: 100%; text-align: center; pointer-events: none; z-index: 30; font-size: 2rem; font-weight: 900; color: #ffd700; text-shadow: 0 0 20px #ffd700, 0 0 5px #fff; opacity: 0; transition: opacity 0.5s, transform 0.5s; transform: translateY(-50px); background: rgba(0,0,0,0.8); padding: 20px 0; border-top: 2px solid #ffd700; border-bottom: 2px solid #ffd700;}
    `;
    ...
    root.innerHTML = `
      <div id="ui-layer">
        ...
        <div id="hud">
          <div class="hud-panel" style="border-color: #00ffcc;" id="xp-panel">
            <div class="hud-label" id="lvl-text" style="color: #00ffcc;">LEVEL 1</div>
            <div class="bar-wrap"><div id="xp-bar"></div></div>
          </div>
          <div class="hud-panel" style="border-color: #ff0044;">
            <div class="hud-label" style="color: #ff0044;">INTEGRITY</div>
            <div class="bar-wrap"><div id="hp-bar"></div></div>
            <div id="hp-text" style="font-size:0.8rem; text-align:right; margin-top:5px; color:#fff; font-weight: bold;">100/100</div>
          </div>
          <div class="hud-panel" style="border-color: #fff; text-align: center; flex: 0.5;">
            <div id="wave-display">WAVE 1/10</div>
            <div id="timer-display">30.0</div>
          </div>
          <div class="hud-panel" style="text-align: right; border-color: #ffd700;">
            <div class="hud-label" style="color: #ffd700;">ENEMIES PURGED</div>
            <div class="hud-value" id="obj-text" style="color: #ffd700; text-shadow: 0 0 15px #ffd700; font-size: 1.6rem;">0</div>
          </div>
        </div>
        <div id="boss-hud">
          <div style="width: 100%; text-align: center;">
            <div id="boss-label">⚠ KRAMPUS-PRIME ⚠</div>
            <div style="display:flex; justify-content:center;">
              <div id="boss-bar-wrap"><div id="boss-hp-bar"></div></div>
            </div>
          </div>
        </div>
        <div id="msg-overlay">WAVE COMPLETE</div>
      ...
      <div id="stick-base" class="stick-base"></div>
      <div id="stick-knob" class="stick-knob"></div>

      <div id="start-screen" class="screen">
        <h1>Protocol: <span>Smart-Tracker</span></h1>
        <h3>10 Waves to Salvation</h3>
        <div class="class-container" id="class-select-container"></div>
        <div style="margin-top: 40px; color: #fff; font-size: 1.1rem; text-align: center; max-width: 600px; line-height: 1.5; font-weight: bold;">
          Survive 10 Waves to secure the Frozen Lake.<br>
          Collect Presents to level up. <span style="color:#ff0044;">Beware Bullet Patterns in later waves.</span><br><br>
          <span style="color: #00ffcc;">DESKTOP: WASD/Arrows to move | SHIFT = Dash</span><br>
          <span style="color: #00ffcc;">MOBILE: Drag anywhere to move</span>
        </div>
      </div>

      <div id="level-screen" class="screen hidden" style="background: rgba(0,20,10,0.95);">
        <h1 style="color: #00ff44; text-shadow: 0 0 30px #00ff44;">FESTIVE UPGRADE <span id="new-level-text" style="color: #fff;">2</span></h1>
        <h3 style="color: #00ffcc;">Select Tactical Enhancement</h3>
        <div class="class-container" id="upgrade-container"></div>
      </div>

      <div id="end-screen" class="screen hidden">
        <h1 id="end-title">MISSION COMPLETE</h1>
        <h3 id="end-msg">The Frozen Lake is secure.</h3>
        <div style="margin: 20px 0; font-size: 1.8rem; color: #fff;">
          WAVES CLEARED: <span id="end-waves" style="color: #00ffcc; font-weight: bold; text-shadow: 0 0 15px #00ffcc;">0/10</span>
        </div>
...
`
    ...
    const terrainFrag = `
      varying float vHeight; varying vec3 vWorldPos;
      void main() {
        vec3 abyss = vec3(0.05, 0.1, 0.2); vec3 floorBase = vec3(0.65, 0.75, 0.85); vec3 gridLine = vec3(0.3, 0.5, 0.7);
        vec3 finalColor = abyss;
        if (vHeight > -3.9) {
          finalColor = mix(abyss, vec3(0.3, 0.5, 0.7), smoothstep(-3.0, 5.0, vHeight));
          finalColor = mix(finalColor, vec3(0.9, 0.95, 1.0), smoothstep(1.0, 6.0, vHeight));
        } else {
          finalColor = floorBase;
          float gridX = smoothstep(0.95, 1.0, fract(vWorldPos.x * 0.5)); float gridZ = smoothstep(0.95, 1.0, fract(vWorldPos.z * 0.5));
          finalColor -= gridLine * (gridX + gridZ) * 0.4;
        }
    ...
    function addWindowListener(event, handler, options) {
      ...
      windowListeners.push({ event, handler, options });
    }

    // Hold game instance for cleanup
    let gameInstance = null;

    // Lazy-load EffectComposer from three/addons
    ...
    class Game {
      ...
      renderClassMenu() {
        ...
        classContainer.innerHTML = `
          <div class="class-card" id="card-elf" style="border-color: #00ffcc;">
            ...
            <div class="card-stat">WEAPON: <span class="stat-val" style="color:#00ffcc">PLASMA MINIGUN</span></div>
          ...
          <div class="class-card ${saveState.unlockedBumble ? '' : 'locked'}" id="card-bumble" style="border-color: #ffd700;">
            ...
            <div class="card-stat">DMG: <span class="stat-val">MAX</span> | ROF: <span class="stat-val">SLOW</span></div>
          </div>
        `;
      ...
      initEngine() {
        ...
        this.scene.fog = new THREE.FogExp2(0x010103, 0.015);

        const w = container.clientWidth || window.innerWidth;
        const h = container.clientHeight || window.innerHeight;

        this.camera = new THREE.PerspectiveCamera(55, w / h, 0.1, 200);
        this.cameraOffset = new THREE.Vector3(0, 45, 20);
      ...
      initProceduralWorld() {
        ...
        for (let x = -CONFIG.WORLD_SIZE / 2; x < CONFIG.WORLD_SIZE / 2; x++) {
          for (let z = -CONFIG.WORLD_SIZE / 2; z < CONFIG.WORLD_SIZE / 2; z++) {
            ...
            dummy.updateMatrix();
            this.terrain.setMatrixAt(idx++, dummy.matrix);
          }
        }
      ...
      selectClass(type) {
        ...
        this.playerSprite = this.createBillboardSprite(type, this.class.scale, this.class.col);
      ...
      upgradeAura() {
        if (this.auraLevel === 0) {
          ...
          this.player.add(this.auraMesh);
        } else {
          this.auraLevel++;
          const newRadius = 3.5 + this.auraLevel * 0.5;
          this.auraMesh.geometry.dispose();
          this.auraMesh.geometry = new THREE.TorusGeometry(newRadius, 0.1, 4, 32);
        }
      }
      ...
      spawnLightning(start, end) {
        ...
        this.scene.add(line);
        this.entities.particles.push(line);
      }
      ...
      animate() {
        ...
        if (this.state === 'PLAYING') {
          ...
          if (this.player) {
            this.playerSprite.quaternion.copy(camQuat);

            if (this.auraLevel > 0 && this.auraMesh) {
              this.auraMesh.rotation.z += dt * 3;
              this.auraTimer += dt;
          ...
          // Physics - Drops
          for (let i = this.entities.drops.length - 1; i >= 0; i--) {
          ...
          for (let i = this.entities.particles.length - 1; i >= 0; i--) {
            ...
            if (p.userData.isLine) {
              p.material.opacity = Math.max(0, p.userData.life / 0.15);
            }
    ...
    // Load postprocessing then start the game
    loadPostProcessing().then(() => {
