...
export default function Game() {
  ...
  useEffect(() => {
    ...
    function createHotWheelsTrackTexture() {
      ...
      ctx.fillRect(64, 0, 15, 1024);
      ctx.fillRect(1024 - 79, 0, 15, 1024);
    ...
    function createGaugeTexture(text) {
      ...
      ctx.fill();
      ctx.fillStyle = "#111";
      ctx.font = "900 45px Courier New";
      ctx.textAlign = "center";
      ctx.fillText(text, 128, 90);
      ctx.translate(128, 128);
      for (let i = 0; i < 8; i++) {
        ctx.fillRect(-4, -110, 8, 25);
    ...
    const obstacleMatRed = new THREE.MeshStandardMaterial({ color: 0xff3e3e, roughness: 0.2 });
    obstacleMatRed.onBeforeCompile = applyCurve;
    const obstacleMatBlue = new THREE.MeshStandardMaterial({ color: 0x00a8ff, roughness: 0.2 });
    obstacleMatBlue.onBeforeCompile = applyCurve;
    ...
    shadowMat.onBeforeCompile = applyCurve;
    ...
    bannerGroup.position.set(0, 0, -1000);
    bannerGroup.visible = false;
    const postGeo = new THREE.CylinderGeometry(1.5, 1.5, 50);
    const post1 = new THREE.Mesh(postGeo, chromeMat);
    post1.position.set(-45, 25, 0);
    bannerGroup.add(post1);
    const post2 = new THREE.Mesh(postGeo, chromeMat);
    post2.position.set(45, 25, 0);
    bannerGroup.add(post2);
    ...
    bCtx.font = "900 120px Courier New";
    bCtx.textAlign = "center";
    bCtx.fillText("FINISH LINE!", 512, 170);
    const bannerMat = new THREE.MeshBasicMaterial({ map: new THREE.CanvasTexture(bCanvas) });
    bannerMat.onBeforeCompile = applyCurve;
    const bannerPlane = new THREE.Mesh(new THREE.PlaneGeometry(90, 18), bannerMat);
    bannerPlane.position.set(0, 42, 0);
    ...
    const arch = new THREE.Mesh(
      new THREE.TorusGeometry(18, 1.2, 16, 64, Math.PI),
    ...
    const dashPanel = new THREE.Mesh(
      new THREE.BoxGeometry(45, 12, 4),
      new THREE.MeshStandardMaterial({ color: 0xcc0000, roughness: 0.8 })
    );
    dashPanel.rotation.x = -0.4;
    dashPanel.position.set(0, -4.5, -2);
    cockpitGroup.add(dashPanel);
    const hood = new THREE.Mesh(
      new THREE.SphereGeometry(16, 32, 32, 0, Math.PI * 2, 0, Math.PI / 2),
      carPaintMat
    );
    hood.scale.set(1.2, 0.35, 1.5);
    hood.position.set(0, -1, -12);
    cockpitGroup.add(hood);

    const wheelGroup = new THREE.Group();
    wheelGroup.position.set(0, -0.5, 0);
    ...
    wheelGroup.add(
      new THREE.Mesh(
        new THREE.TorusGeometry(3.5, 0.6, 16, 32),
        new THREE.MeshStandardMaterial({ color: 0x9c27b0, roughness: 0.2 })
      )
    );
    wheelGroup.add(new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 6.8), chromeMat));
    const spoke2 = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.2, 6.8), chromeMat);
    spoke2.rotation.z = Math.PI / 2;
    wheelGroup.add(spoke2);
    const horn = new THREE.Mesh(
      new THREE.CylinderGeometry(1.4, 1.6, 0.6, 32),
      new THREE.MeshStandardMaterial({ color: 0xff3e3e, roughness: 0.2 })
    );
    ...
    const createGauge = (text, xPos) => {
      ...
      gGroup.position.set(xPos, -2.0, -0.5);
      gGroup.rotation.x = -0.4;
      const bezel = new THREE.Mesh(new THREE.CylinderGeometry(2.2, 2.2, 0.4, 32), chromeMat);
      bezel.rotation.x = Math.PI / 2;
      gGroup.add(bezel);
      const face = new THREE.Mesh(
        new THREE.PlaneGeometry(4.0, 4.0),
        new THREE.MeshBasicMaterial({ map: createGaugeTexture(text) })
      );
      face.position.z = 0.21;
      gGroup.add(face);
      const needle = new THREE.Mesh(
        new THREE.BoxGeometry(0.15, 1.6, 0.08),
        new THREE.MeshBasicMaterial({ color: 0xff0000 })
      );
      needle.position.set(0, 0, 0.25);
      needle.geometry.translate(0, 0.8, 0);
      gGroup.add(needle);
      cockpitGroup.add(gGroup);
      return needle;
    };
    const needleRPM = createGauge("RPM", -8);
    const needleSpeed = createGauge("MPH", 8);

    const mirrorGroup = new THREE.Group();
    mirrorGroup.position.set(0, 16, -2);
    cockpitGroup.add(mirrorGroup);
    mirrorGroup.add(
      new THREE.Mesh(
        new THREE.BoxGeometry(6, 2.0, 0.4),
        new THREE.MeshStandardMaterial({ color: 0x444444 })
      )
    );
    const glass = new THREE.Mesh(
      new THREE.PlaneGeometry(5.6, 1.6),
      new THREE.MeshStandardMaterial({ color: 0x888888, metalness: 1.0, roughness: 0.0 })
    );
    glass.position.z = 0.21;
    mirrorGroup.add(glass);
    const diceGroup = new THREE.Group();
    diceGroup.position.set(1.5, -1, 0);
    mirrorGroup.add(diceGroup);
    diceGroup.add(
      new THREE.Mesh(
    ...
    const d1 = new THREE.Mesh(
      ...
      new THREE.MeshStandardMaterial({ color: 0x00a8ff, roughness: 0.8 })
    );
    d1.position.set(-0.3, -3.0, 0);
    d1.rotation.set(1, 2, 3);
    diceGroup.add(d1);
    const d2 = new THREE.Mesh(
      new THREE.BoxGeometry(1, 1, 1),
      new THREE.MeshStandardMaterial({ color: 0xff3e3e, roughness: 0.8 })
    );
    d2.position.set(0.5, -2.4, 0.5);
    d2.rotation.set(3, 2, 1);
    diceGroup.add(d2);

    // === UI OVERLAYS ===
    const hud = document.createElement("div");
    hud.style.cssText =
      "display:none;position:absolute;top:20px;left:20px;color:white;font-size:clamp(18px,5vw,28px);font-weight:900;text-shadow:2px 2px 0 #000,-2px -2px 0 #000,2px -2px 0 #000,-2px 2px 0 #000;pointer-events:none;z-index:10;font-family:Courier New,monospace;";
    hud.innerHTML =
      'DIST: <span id="mt-dist" style="color:#ffd700;">0</span>m<br>SPEED: <span id="mt-speed" style="color:#ffd700;">0</span> mph';
    ...
    const btnStyle =
      "padding:20px 60px;font-size:28px;font-weight:900;background:#ff3e3e;color:white;border:4px solid white;border-radius:15px;cursor:pointer;text-transform:uppercase;font-family:Courier New,monospace;";
    const startScreen = mkOverlay(
      "mt-start",
      `<h1 style="font-size:clamp(50px,12vw,80px);color:#ff3e3e;text-shadow:4px 4px 0 #fff;margin-bottom:10px;text-align:center;">MEGA TRACK</h1><p style="font-size:clamp(20px,6vw,28px);color:#ffd700;margin-bottom:40px;text-align:center;">Avoid obstacles. Reach the end.</p><button id="mt-start-btn" style="${btnStyle}">Start Engine</button>`
    );
    const gameOverScreen = mkOverlay(
      "mt-gameover",
      `<h1 id="mt-title" style="font-size:clamp(50px,12vw,80px);color:#ff3e3e;text-shadow:4px 4px 0 #fff;margin-bottom:10px;text-align:center;">WIPEOUT!</h1><p id="mt-desc" style="font-size:clamp(20px,6vw,28px);color:#ffd700;margin-bottom:40px;text-align:center;">You hit an obstacle.</p><button id="mt-restart-btn" style="${btnStyle.replace("background:#ff3e3e", "background:#00a8ff").replace("#cc0000", "#0055ff")}">Drive Again</button>`,
      "none"
    );
    ...
    function handleResize() {
      ...
      if (w / h < 1) {
        camera.fov = 90;
        cockpitGroup.position.set(0, -2.8, -6);
        cockpitGroup.scale.setScalar(0.22);
      } else {
        camera.fov = 75;
        cockpitGroup.position.set(0, -2.2, -6);
        cockpitGroup.scale.setScalar(0.25);
    ...
    function spawnObstacle() {
      ...
      const obs = new THREE.Mesh(obsGeo, mat);
      obs.castShadow = true;
      ...
      shadow.position.y = isBox ? -3.9 : -4.9;
      obs.add(shadow);
    ...
    function gameOver(won) {
      ...
      setTimeout(() => {
        ...
        const title = getElementById("mt-title");
        const desc = getElementById("mt-desc");
        const btn = getElementById("mt-restart-btn");
        if (title) {
          title.innerText = won ? "YOU WIN!" : "WIPEOUT!";
          title.style.color = won ? "#ffd700" : "#ff3e3e";
        }
        if (desc)
          desc.innerText = won
            ? "Crossed the finish line!"
            : `Crashed at ${Math.floor(distance)}m.`;
    ...
    function createHonkEffect(x, y) {
      ...
      setTimeout(() => {
        if (honk.parentNode) honk.parentNode.removeChild(honk);
      }, 600);
    }
    ...
    styleEl.textContent =
      "@keyframes floatUp { 0%{opacity:1;transform:translate(-50%,0) scale(0.5) rotate(-10deg)} 50%{transform:translate(-50%,-50px) scale(1.2) rotate(10deg)} 100%{opacity:0;transform:translate(-50%,-100px) scale(1) rotate(0deg)} }";
    ...
    const baseRigPos = new THREE.Vector3(0, 0, 0);
    let animFrameId;
    ...
    function animate() {
      ...
      if (isPlaying) {
        ...
        const distEl = getElementById("mt-dist");
        const speedEl = getElementById("mt-speed");
        ...
        for (let i = obstacles.length - 1; i >= 0; i--) {
          ...
          if (obs.geometry.type === "BoxGeometry") {
            obs.rotation.x -= moveStep * 0.03;
            obs.rotation.y += moveStep * 0.02;
          } else {
            obs.rotation.x -= moveStep * 0.05;
          }
          if (obs.userData.active && obs.position.z > 0 && obs.position.z < 15) {
            if (Math.abs(obs.position.x - carRig.position.x) < 8.5) {
      ...
      if (shakeIntensity > 0) {
        ...
        if (shakeIntensity < 0.01) {
          shakeIntensity = 0;
          carRig.position.copy(baseRigPos);
        }
      }
