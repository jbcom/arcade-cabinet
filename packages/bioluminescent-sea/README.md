...
The scene uses a parent-sized canvas, particulate drift, authored beacon lanes, and a deep vertical gradient so it can run fullscreen, inside the Astro cabinet, or inside an Android WebView without assuming `window.innerHeight`.
