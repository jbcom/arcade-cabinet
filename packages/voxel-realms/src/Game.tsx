...
export default function Game() {
  ...
  useEffect(() => {
    ...
    function updateHUD() {
      uiHealth.innerHTML =
        "❤️".repeat(gameState.health) + "🖤".repeat(gameState.maxHealth - gameState.health);
    ...
    function takeDamage(amt) {
      ...
      setTimeout(() => {
        if (!gameState.isDead) uiVignette.style.opacity = 0;
      }, 300);
    ...
    function respawn() {
      ...
      uiDeathScreen.classList.remove("active");
      uiVignette.style.opacity = 0;
      creatures.forEach((c) => {
        scene.remove(c);
      });
      creatures.length = 0;
    ...
    const materials = {
      ...
      slime: new THREE.MeshPhongMaterial({
        color: 0x00d4ff,
        transparent: true,
        opacity: 0.85,
        shininess: 100,
      }),
      slimeAggro: new THREE.MeshPhongMaterial({
        color: 0xff3333,
        transparent: true,
        opacity: 0.9,
        shininess: 100,
    ...
    function getProceduralHeight(wx, wz) {
      let h = Math.sin(wx * 0.08) * 3 + Math.cos(wz * 0.08) * 3;
      h += Math.sin(wx * 0.2) * 1.5;
      return Math.floor(h);
    }
    ...
    function setPlayerStartLocation() {
      let sx = 0,
        sz = 0,
        sy = getProceduralHeight(0, 0);
      let radius = 1;
      while (sy <= 0 && radius < 50) {
        for (let i = -radius; i <= radius; i++) {
          for (let j = -radius; j <= radius; j++) {
            const tempY = getProceduralHeight(i, j);
            if (tempY > 0) {
              sx = i;
              sz = j;
              sy = tempY;
    ...
    function generateChunk(cx, cz) {
      ...
      for (let lx = 0; lx < CHUNK_SIZE; lx++) {
        for (let lz = 0; lz < CHUNK_SIZE; lz++) {
          ...
          if (wy < -1) {
            chunkBlueprint.set(`${lx},${-1},${lz}`, "water");
            chunkBlueprint.set(`${lx},${wy},${lz}`, "sand");
          } else if (wy === -1 || wy === 0) {
            chunkBlueprint.set(`${lx},${wy},${lz}`, "sand");
            chunkBlueprint.set(`${lx},${wy - 1},${lz}`, "stone");
          } else if (wy > 4) {
            chunkBlueprint.set(`${lx},${wy},${lz}`, "snow");
            chunkBlueprint.set(`${lx},${wy - 1},${lz}`, "stone");
          } else {
            chunkBlueprint.set(`${lx},${wy},${lz}`, "grass");
            chunkBlueprint.set(`${lx},${wy - 1},${lz}`, "dirt");
            chunkBlueprint.set(`${lx},${wy - 2},${lz}`, "stone");
            const treeSeed = Math.sin(wx * 12.9898 + wz * 78.233);
            if (treeSeed > 0.97 && lx > 1 && lx < CHUNK_SIZE - 2 && lz > 1 && lz < CHUNK_SIZE - 2) {
              chunkBlueprint.set(`${lx},${wy + 1},${lz}`, "wood");
              chunkBlueprint.set(`${lx},${wy + 2},${lz}`, "wood");
              chunkBlueprint.set(`${lx},${wy + 3},${lz}`, "leaves");
              chunkBlueprint.set(`${lx + 1},${wy + 3},${lz}`, "leaves");
              chunkBlueprint.set(`${lx - 1},${wy + 3},${lz}`, "leaves");
              chunkBlueprint.set(`${lx},${wy + 3},${lz + 1}`, "leaves");
              chunkBlueprint.set(`${lx},${wy + 3},${lz - 1}`, "leaves");
              chunkBlueprint.set(`${lx},${wy + 4},${lz}`, "leaves");
            }
          }
        }
      }
      ...
      if (deltaMemory.has(chunkKey)) {
        for (const [blockKey, type] of deltaMemory.get(chunkKey).entries()) {
          if (type === "air") chunkBlueprint.delete(blockKey);
          else chunkBlueprint.set(blockKey, type);
        }
      }
    ...
    function updateChunks() {
      ...
      for (let dx = -RENDER_DIST; dx <= RENDER_DIST; dx++) {
        for (let dz = -RENDER_DIST; dz <= RENDER_DIST; dz++)
          requiredChunks.add(`${playerCX + dx},${playerCZ + dz}`);
      }
      ...
      for (const [chunkKey, chunkGroup] of activeChunks.entries()) {
        if (!requiredChunks.has(chunkKey)) {
          ...
          chunkGroup.children.forEach((mesh) => {
            ...
            if (mesh.userData.type === "torch") activeTorches.delete(mesh);
          });
          activeChunks.delete(chunkKey);
        }
      }
      for (const chunkKey of requiredChunks) {
        if (!activeChunks.has(chunkKey)) generateChunk(...chunkKey.split(",").map(Number));
      }
    }
    ...
    leftZone.addEventListener("pointerdown", (e) => {
      ...
      joyBase.style.left = `${leftStartX}px`;
      joyBase.style.top = `${leftStartY}px`;
    ...
    const onPointerMoveLeft = (e) => {
      if (e.pointerId === leftTouchId) {
        let dx = e.clientX - leftStartX;
        let dy = e.clientY - leftStartY;
        ...
        inputState.touchMoveX = dx / joyMaxRadius;
        inputState.touchMoveY = dy / joyMaxRadius;
      }
    };
    ...
    window.addEventListener("pointerup", endLeftTouch);
    window.addEventListener("pointercancel", endLeftTouch);
    ...
    window.addEventListener("pointerup", endRightTouch);
    window.addEventListener("pointercancel", endRightTouch);
    ...
    function showLog(text) {
      logUI.textContent = text;
      logUI.style.opacity = 1;
      clearTimeout(logTimeout);
      logTimeout = setTimeout(() => {
        logUI.style.opacity = 0;
      }, 1500);
    }
    ...
    function attemptMine() {
      ...
      recoilActive = 0.1;
      if (targetBlock) {
        if (targetBlock.userData.type === "water") {
          return;
        }
        if (targetBlock.userData.isCreature) {
          createDebris(targetBlock.position, targetBlock.material, 20);
          scene.remove(targetBlock);
          creatures.splice(creatures.indexOf(targetBlock), 1);
          addScore(10);
          showLog("+10 Mana Gem!");
        } else {
          ...
          saveDelta(
            targetBlock.userData.cx,
            targetBlock.userData.cz,
            targetBlock.userData.localKey,
            "air"
          );
        ...
        highlightMesh.visible = false;
        crosshairUI.classList.remove("enemy");
        targetBlock = null;
      }
    }
    ...
    function attemptBuild() {
      ...
      if (targetBlock && targetFaceNormal && !targetBlock.userData.isCreature) {
        ...
        const ncx = Math.floor(wPos.x / CHUNK_SIZE);
        const ncz = Math.floor(wPos.z / CHUNK_SIZE);
        const lx = wPos.x - ncx * CHUNK_SIZE;
        const lz = wPos.z - ncz * CHUNK_SIZE;
    ...
    let velocityY = 0;
    const defaultGravity = -0.012;
    let currentGravity = defaultGravity;
    const knockbackVel = new THREE.Vector3();
    function attemptJump() {
      if (gameState.isDead) return;
      const downRay = new THREE.Raycaster(player.position, new THREE.Vector3(0, -1, 0), 0, 2.5);
      if (downRay.intersectObject(terrainGroup, true).length > 0 || isUnderwater) {
        velocityY = isUnderwater ? 0.1 : 0.22;
      }
    }
    ...
    container.querySelector("#btn-mine").addEventListener("pointerdown", (e) => {
      e.stopPropagation();
      attemptMine();
    });
    container.querySelector("#btn-place").addEventListener("pointerdown", (e) => {
      e.stopPropagation();
      attemptBuild();
    });
    container.querySelector("#btn-jump").addEventListener("pointerdown", (e) => {
      e.stopPropagation();
      attemptJump();
    });
    container.querySelector("#btn-crouch").addEventListener("pointerdown", (e) => {
      e.stopPropagation();
      if (!gameState.isDead) inputState.isCrouching = true;
    });
    container
      .querySelector("#btn-crouch")
      .addEventListener("pointerup", () => (inputState.isCrouching = false));
    ...
    // --- 6. RENDER & GAME LOOP ---
    const defaultMoveSpeed = 0.15;
    let currentMoveSpeed = defaultMoveSpeed;
    let bobTimer = 0;
    ...
    function animate() {
      ...
      if (gameState.dayTime > Math.PI * 2.25) {
        gameState.dayTime = Math.PI / 4;
        gameState.dayCount++;
        addScore(50);
        if (gameState.health < gameState.maxHealth) {
          gameState.health++;
          updateHUD();
          showLog(`☀️ Day ${gameState.dayCount} Begins! (+1 Heart)`);
        } else {
          showLog(`☀️ Day ${gameState.dayCount} Begins!`);
        }
      }

      const sunDistance = 50;
      dirLight.position.set(
        Math.cos(gameState.dayTime) * sunDistance,
        Math.sin(gameState.dayTime) * sunDistance,
        20
      );
      const sunHeight = Math.sin(gameState.dayTime);
      const mixRatio = Math.max(0, Math.min(1, (sunHeight + 0.2) / 0.4));

      const headHeight = camera.getWorldPosition(new THREE.Vector3()).y;
      isUnderwater = headHeight < -0.2;

      if (isUnderwater) {
        scene.background.lerp(colorUnderwater, 0.1);
        scene.fog.color.copy(scene.background);
        scene.fog.near = THREE.MathUtils.lerp(scene.fog.near, 1, 0.1);
        scene.fog.far = THREE.MathUtils.lerp(scene.fog.far, 10, 0.1);
        currentGravity = defaultGravity * 0.2;
        currentMoveSpeed = defaultMoveSpeed * 0.5;
      } else {
        scene.background.copy(colorNight).lerp(colorDay, mixRatio);
        scene.fog.color.copy(scene.background);
        scene.fog.near = THREE.MathUtils.lerp(scene.fog.near, 8, 0.1);
        scene.fog.far = THREE.MathUtils.lerp(scene.fog.far, 30, 0.1);
        currentGravity = defaultGravity;
        currentMoveSpeed = defaultMoveSpeed;
      }

      ambientLight.intensity = 0.1 + mixRatio * 0.4;
      dirLight.intensity = Math.max(0, sunHeight) * 1.5;
      starsMat.opacity = 1.0 - mixRatio;
      ...
      const sortedTorches = Array.from(activeTorches).sort(
        (a, b) =>
          a.getWorldPosition(new THREE.Vector3()).distanceToSquared(player.position) -
          b.getWorldPosition(new THREE.Vector3()).distanceToSquared(player.position)
      );
      for (let i = 0; i < 4; i++) {
        if (i < sortedTorches.length) {
          torchLights[i].position.copy(sortedTorches[i].getWorldPosition(new THREE.Vector3()));
      ...
      if (recoilActive !== 0) {
        recoilActive = THREE.MathUtils.lerp(recoilActive, 0, 0.2);
        if (Math.abs(recoilActive) < 0.001) recoilActive = 0;
      }
      player.rotation.y = lookYaw;
      camera.rotation.x = lookPitch + recoilActive;

      if (!gameState.isDead) {
        let netMoveX = inputState.touchMoveX + (keys.d ? 1 : 0) - (keys.a ? 1 : 0);
        let netMoveY = inputState.touchMoveY + (keys.s ? 1 : 0) - (keys.w ? 1 : 0);
        const netMag = Math.sqrt(netMoveX * netMoveX + netMoveY * netMoveY);
        if (netMag > 1) {
          netMoveX /= netMag;
          netMoveY /= netMag;
        }
        ...
        player.position.addScaledVector(forward, -netMoveY * currentMoveSpeed);
        player.position.addScaledVector(right, netMoveX * currentMoveSpeed);
      }

      player.position.add(knockbackVel);
      knockbackVel.lerp(new THREE.Vector3(0, 0, 0), 0.1);
      ...
      terrainGroup.children.forEach((chunk) =>
        chunk.children.forEach((mesh) => {
          if (mesh.userData.type !== "water") solidMeshes.push(mesh);
        })
      );
      ...
      if (groundHits.length > 0 && groundHits[0].distance < targetHeightOffset + 0.5) {
        player.position.y = groundHits[0].point.y + targetHeightOffset;
        velocityY = 0;
        if (
          !gameState.isDead &&
          (inputState.touchMoveX || inputState.touchMoveY || keys.w || keys.a || keys.s || keys.d)
        ) {
          bobTimer += isUnderwater ? 0.15 : 0.25;
      ...
      for (let i = 0; i < creatures.length; i++) {
        const c = creatures[i];
        c.userData.velocityY += currentGravity * 0.8;
        c.position.y += c.userData.velocityY;
        const slimeRay = new THREE.Raycaster(
          c.position.clone().add(new THREE.Vector3(0, 0.5, 0)),
          new THREE.Vector3(0, -1, 0)
        );
        const sHits = slimeRay.intersectObjects(solidMeshes, false);
        const distToPlayer = c.position.distanceTo(player.position);

        if (distToPlayer < 20 && !gameState.isDead) {
          c.userData.isAggro = true;
          c.material = materials.slimeAggro;
        } else {
          c.userData.isAggro = false;
          c.material = materials.slime;
        }
        ...
        if (
          c.userData.isAggro &&
          distToPlayer < 1.5 &&
          c.userData.dmgCooldown <= 0 &&
          !gameState.isDead
        ) {
          takeDamage(1);
          c.userData.dmgCooldown = 60;
          const kbDir = player.position.clone().sub(c.position).normalize();
          knockbackVel.set(kbDir.x * 0.4, 0.2, kbDir.z * 0.4);
        }

        if (sHits.length > 0 && sHits[0].distance < 0.9) {
          c.position.y = sHits[0].point.y + 0.4;
          c.userData.velocityY = 0;
          c.userData.moveDir.set(0, 0, 0);
          c.scale.set(
            THREE.MathUtils.lerp(c.scale.x, 1, 0.2),
            THREE.MathUtils.lerp(c.scale.y, 1, 0.2),
            THREE.MathUtils.lerp(c.scale.z, 1, 0.2)
          );

          c.userData.jumpTimer--;
          if (c.userData.jumpTimer <= 0) {
            c.userData.velocityY = 0.15 + Math.random() * 0.05;
            const speed = 0.04 + Math.random() * 0.04;
            if (c.userData.isAggro) {
              const dir = player.position.clone().sub(c.position).normalize();
              c.userData.moveDir.set(dir.x * speed * 1.5, 0, dir.z * speed * 1.5);
              c.userData.jumpTimer = 20 + Math.random() * 40;
            } else {
              const angle = Math.random() * Math.PI * 2;
              c.userData.moveDir.set(Math.cos(angle) * speed, 0, Math.sin(angle) * speed);
              c.userData.jumpTimer = 40 + Math.random() * 80;
            }
          }
        } else {
          c.position.add(c.userData.moveDir);
          const stretch = 1 + Math.abs(c.userData.velocityY) * 3;
          const squash = 1 - Math.abs(c.userData.velocityY) * 1.5;
          c.scale.set(squash, stretch, squash);
        }
      }
      ...
      if (!gameState.isDead) {
        ...
        const intersects = raycaster
          .intersectObjects(targetables, true)
          .filter((hit) => hit.distance < 6);

        if (intersects.length > 0) {
          targetBlock = intersects[0].object;
          targetFaceNormal = intersects[0].face.normal;
          if (targetBlock.userData.isCreature) {
            highlightMesh.scale.set(0.8, targetBlock.scale.y * 0.8, 0.8);
            highlightMesh.material.color.setHex(0xff3333);
            crosshairUI.classList.add("enemy");
          } else {
            highlightMesh.scale.set(1, 1, 1);
            highlightMesh.material.color.setHex(0xffd700);
            crosshairUI.classList.remove("enemy");
          }
          highlightMesh.position.copy(targetBlock.getWorldPosition(new THREE.Vector3()));
          highlightMesh.visible = true;
          crosshairUI.classList.add("active");
        } else {
          targetBlock = null;
          targetFaceNormal = null;
          highlightMesh.visible = false;
          crosshairUI.classList.remove("active", "enemy");
        }
      } else {
        highlightMesh.visible = false;
        crosshairUI.classList.remove("active", "enemy");
      ...
      for (let i = particles.length - 1; i >= 0; i--) {
        ...
        p.userData.velocity.y += currentGravity * 0.8;
        p.scale.multiplyScalar(isUnderwater ? 0.96 : 0.92);
        if (p.scale.x < 0.05) {
          scene.remove(p);
