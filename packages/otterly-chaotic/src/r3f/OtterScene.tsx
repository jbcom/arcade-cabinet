...
function Character({ position, color, scale = 1 }: { position: [number, number, number]; color: string; scale?: number }) {
  return (
...
export function OtterScene({ state }: OtterSceneProps) {
  return (
    <Canvas shadows camera={{ position: [0, 8.5, 7], fov: 48 }} style={{ width: '100%', height: '100%' }} data-testid="otterly-chaotic-canvas">
      ...
      <mesh position={[WATER_ZONE.x + WATER_ZONE.width / 2, 0.02, WATER_ZONE.z + WATER_ZONE.depth / 2]} rotation={[-Math.PI / 2, 0, 0]} receiveShadow>
      ...
      {state.goats.map((goat) => (
        <group key={goat.id}>
          <Character position={[goat.position.x, 0.1, goat.position.y]} color={goat.stunnedMs > 0 ? '#c084fc' : '#e2e8f0'} scale={goat.id === 'elder' ? 1.15 : 1} />
        </group>
      ))}
      {[[1.4, -2.8], [2.5, -3.2], [-3.3, 2.4], [0.8, 2.2]].map(([x, z], index) => (
        <mesh key={index} castShadow receiveShadow position={[x, 0.28, z]}>
          <cylinderGeometry args={[0.35, 0.5, 0.55, 8]} />
          <meshStandardMaterial color="#475569" />
        </mesh>
      ))}
      <OrbitControls enablePan={false} maxPolarAngle={1.2} minDistance={7} maxDistance={11} target={[0, 0.5, 0]} />
    </Canvas>
  );
}
