...
import { titanWorld, titanEntity } from "./store/world";
...
function TitanApp() {
  ...
  const handleStart = () => {
    ...
    titanEntity.set(TitanTrait, { phase: "playing", hp: 200, maxHp: 200, energy: 100, maxEnergy: 100, scrap: 0, score: 0 });
  };
    ...
    <div style={{ width: "100%", height: "100svh", position: "relative", background: "#020617" }}>
      ...
      {phase === "menu" && (
        <div className="absolute inset-0 flex flex-col items-center justify-center bg-[#020617]/90 backdrop-blur-md z-20 text-[#00ffcc] font-mono">
          <h1 className="text-5xl md:text-7xl font-black uppercase tracking-[10px] mb-8 text-center" style={{ textShadow: "0 0 20px rgba(0,255,204,0.5)" }}>
            TITAN MECH OS
          </h1>
          <div className="bg-black/60 p-8 rounded border border-[#00ffcc]/30 max-w-xl text-left leading-relaxed mb-10">
            <div className="text-xs opacity-50 mb-4 tracking-widest">BOOT SEQUENCE INITIALIZED...</div>
            [W][A][S][D] - Pilot the chassis<br/>
            [MOUSE] - Target acquisition<br/>
            [LEFT CLICK] - Discharge ordinance<br/>
            [ESC] - System pause<br/><br/>
            <span className="text-amber-400 font-bold">OBJECTIVE:</span> Neutralize hostile constructs. Accumulate scrap for hardware iterations.
          </div>
          <button 
            onClick={handleStart}
