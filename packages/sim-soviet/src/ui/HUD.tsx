...
export function HUD({ state, onSelectTool }: HUDProps) {
  return (
    <HUDOverlay
      topLeft={
        <div>
          <div style={{ fontSize: 12, letterSpacing: '0.22em', textTransform: 'uppercase', color: '#facc15' }}>Sim Soviet 3000</div>
          <h2 style={{ margin: '0.35rem 0', fontSize: 28 }}>Five-Year Plan Control</h2>
          <div style={{ color: '#cbd5e1' }}>Place sectors to grow housing, food, and heavy industry.</div>
        </div>
      }
      topRight={
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: '0.35rem 0.75rem' }}>
          ...
          <span>{state.month.toString().padStart(2, '0')}/{state.year}</span>
        </div>
      }
      ...
      bottomRight={
        <div>
          ...
          <div style={{ height: 10, borderRadius: 999, background: 'rgba(15, 23, 42, 0.6)', overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${state.quotaProgress}%`, background: 'linear-gradient(90deg, #22c55e, #facc15)' }} />
          </div>
