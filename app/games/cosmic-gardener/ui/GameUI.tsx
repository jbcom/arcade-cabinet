...
export function GameUI({
  ...
  return (
    <>
      ...
      {lowerBoardLayout.lowerHudVariant === "upper-strip" ? (
        <motion.div
          className="pointer-events-none absolute left-4 right-20 top-[5.85rem] z-50 grid grid-cols-2 gap-2 rounded-lg border border-white/10 bg-slate-950/48 p-2 shadow-2xl shadow-black/25 backdrop-blur-md"
        ...
        >
          <CompactGauge
            ...
            gradient="linear-gradient(90deg, #fbbf24, #f59e0b, #ec4899)"
          />
          <CompactGauge
            align="right"
            label="Warmth"
          ...
          >
            ...
            <span
              className={cn(
                ...
                isColdDanger ? "text-red-400" : "text-white/80"
              )}
            >
...
function CompactGauge({
  ...
  return (
    <div className={cn("grid gap-1", align === "right" && "text-right")}>
      <div className="flex items-baseline justify-between gap-2 text-[0.56rem] font-bold uppercase tracking-[0.16em] text-white/55">
        <span>{label}</span>
        <span className={danger ? "text-red-300" : "text-white/80"}>{value}</span>
      </div>
