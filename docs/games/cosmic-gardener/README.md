...
- The all-constellations finish now exposes a deterministic Zen Garden Bloom cue with bloom-ring intensity, completion label, and replay promise before free cultivation.
...
- `src/engine/constellationProgress.ts` owns next-pattern preview and final zen transition cue copy/intensity so end-of-run presentation remains testable.
