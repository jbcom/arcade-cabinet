...
| Cosmic Gardener | Landing label has a distinct pinball-garden voice; gameplay remains strong in desktop and portrait with visible save count, recovery valve language, and a defined all-pattern finish. | Added deterministic recovery bloom ball saves, recovery pulse feedback, completion bonus/next-pattern preview, resonance bloom, launch/drain pulses, constellation edge completion feedback, deterministic Zen Garden Bloom cue/overlay, and a compact narrow-portrait lower-board layout that moves gauges out of the flipper apron. | Add optional late-pattern table dressing after the zen finish is playtested. |
| Enchanted Forest | Landing, grove stage, and mobile rune scale now feel coherent; latest gameplay captures show a visible rune cue, spell cadence strip, focus ring, shadow path, and target tree read on desktop and mobile. | Added deterministic ritual cue telemetry, deterministic spell cadence cues, desktop/mobile rune cue HUD, stage focus rings, target-tree ritual lighting, standard/cozy opening damage grace, harmony surge documentation, audio-status visual fallback, shadow path metadata, per-spell activation motifs, and larger rune affordances. | Add optional later-wave grove set dressing after the core spell grammar is playtested. |
...
| Otterly Chaotic | Landing now matches the pasture-chase identity; gameplay desktop and mobile stay active during capture with a visible next-action cue, bark radius ring, and more expressive animal poses. | Added deterministic rescue cue telemetry, otter/goat pose cue telemetry, HUD next-action/round/pose cues, arena cue rings, goat intent indicators, bark shockwave, stun stars, pose rings, tail/hoof/head pose adjustments, goal pull line, salad feedback, and a longer playable opening window. | Add optional arena event variants after the core rescue loop is locked. |
...
| Titan Mech: Overheat | Landing label now frames the game as an extraction cartridge; gameplay exposes route, contract, heat, pylon state, threat behavior, delivery state, and upgrade choices in the same cabinet frame. | Added contract cue state, page-mode screenshots, active route chevrons, pylon beams, stronger extractor glow, threat markers, attack-lane beams, behavior-specific enemy telegraphs, HUD contract/delivery/threat stages, weapon feedback, ore rigs, sustained payout cube, reticle, damage vignette, deterministic hostile pressure, and post-contract upgrade choices. | Add optional contract modifier cards after the upgraded multi-contract loop is playtested. |
| Beppo Laughs | Landing reads as a circus cartridge and gameplay now presents the route as a stage with lit curtains, room-specific motifs, lighting beats, and more distinct ending payoffs. | Added pure route cue state, room cue state, ending cue state, memory/missing-item/recommended-curtain telemetry, circus-stage play surface, portal buttons, cue lighting, room-specific spotlight/prop motifs, route memory pips, and clean-route/panic/loss ending overlays. | Add optional room-event variants after the core maze identity is locked. |
| Cognitive Dissonance | Landing and gameplay now read as a diegetic AI-cabinet cartridge with glass frame, scanlines, rim nodes, pattern rain, compact portrait HUD, and distinct stable/shatter endings. | Added phase-lock recovery, deterministic shift cue state, deterministic runtime feedback cue state, cabinet-glass Three presentation, phase-lock halo feedback, next-pattern HUD text, mobile HUD compression, glass-lock rings, fracture-shard ending payoffs, and explicit visual fallback labels when audio or haptics are unavailable. | Tune full-shift pacing after the runtime feedback pass. |
