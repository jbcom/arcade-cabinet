...
| Entropy Edge | Landing label and gameplay now read as a failing machine table with clear cyan route beacons, red blocked hazards, and a portrait-framed sector path. | Added deterministic sector cue state, HUD objective/pressure routing, route beacons, hazard markings, responsive camera framing, resonance surge payoff, resonance bands, and corrected Three opacity usage. | Add a more dramatic sector-stabilized win transition. |
| Mega Track | Landing and gameplay now read as a three-leg toy-scale cup with leg HUD, safe-lane callouts, checkpoint gates, visible hazard previews, and portrait framing that keeps the car in view. | Added checkpoint repair/boost recovery, deterministic race cue state, next-hazard HUD text, safe-lane overlays, repair pulses, overdrive/impact/clean-pass telemetry, lane strips, speed markings, collision sparks, and hazard bases. | Add authored track-side scenery variation by distance band. |
| Overcast: Glacier | Reduced cartridge now reads as a named glacier route with weather, lane warnings, and richer collectible/threat tokens. | Added segment cue state, named route gates, weather bands, lane warning overlays, richer cocoa/snowman/glitch shapes, kitten silhouette details, and segment HUD text. | Tune late-segment entity density and add stronger finish-line celebration. |
...
| Titan Mech: Overheat | Landing label now frames the game as an extraction cartridge; gameplay exposes route, contract, heat, and pylon state in the same cabinet frame. | Added contract cue state, page-mode screenshots, active route chevrons, pylon beams, stronger extractor glow, threat markers, HUD contract stage, weapon feedback, ore rigs, reticle, and damage vignette. | Add enemy attack readability, visible cube delivery timing, and a clearer victory state. |
...
| Cognitive Dissonance | Landing and gameplay now read as a diegetic AI-cabinet cartridge with glass frame, scanlines, rim nodes, pattern rain, and compact portrait HUD. | Added phase-lock recovery, deterministic shift cue state, cabinet-glass Three presentation, phase-lock halo feedback, next-pattern HUD text, and mobile HUD compression. | Add more visible stable/shatter ending animations. |
| Farm Follies | Landing label has the strongest cartridge readability of the new imports; gameplay now shows best-lane forecasting, a ghost next-drop token, readable bank action, and real merged animal tokens. | Added deterministic stack cue state, best-lane/merge preview HUD, bank-ready metric, lane highlight, ghost token forecast, pair-friendly opening drops, deterministic merge abilities, animal silhouettes/tier badges, sway/danger meter, and ability feedback. | Add collapse animation and more late-tier animal poses. |
